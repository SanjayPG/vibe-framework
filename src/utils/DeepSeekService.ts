...
/**
 * DeepSeek service for parsing natural language commands
...
export class DeepSeekService implements AIService {
  ...
  async parseCommand(request: ParseRequest): Promise<ParseResponse> {
    ...
    try {
      const { content, usage } = await this.callDeepSeekAPI(prompt);
...
Return JSON format:
{
  ...
  private async callDeepSeekAPI(prompt: string): Promise<{ content: string; usage: any }> {
    ...
    const usage = data.usage; // Extract token usage (OpenAI-compatible format)
  ...
  private parseResponse(responseText: string, originalCommand: string, usage?: any): ParseResponse {
    try {
      ...
      throw new Error(`Failed to parse DeepSeek response: ${error.message}. Response: ${responseText}`);
    }
  }
}
